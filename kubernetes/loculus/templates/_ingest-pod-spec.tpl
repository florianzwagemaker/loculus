...
spec:
  ...
  {{- include "loculus.podSecurityContext" (dict "componentName" "ingest" "Values" $Values) | nindent 2 }}
  ...
  initContainers:
    - name: version-check
      ...
      {{- include "loculus.containerSecurityContext" (dict "componentName" "ingest-init" "Values" $Values) | nindent 6 }}
      command: ['sh', '-c', '
  ...
  containers:
    - name: ingest-{{ $key }}
      ...
      {{- include "loculus.containerSecurityContext" (dict "componentName" "ingest" "Values" $Values) | nindent 6 }}
      env:
